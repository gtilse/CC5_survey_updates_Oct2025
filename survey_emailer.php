...
function sendClientSurvey(){
  ...
  if($emailType==$GLOBALS["EMAIL_TYPE"]["SURVEY"]) {         //Survey
    $sql = "SELECT Client.objectId as clientId, Client.vendorId, Client.title, Client.name, Client.organisation, Client.email, Client.drl," .
           ...
           "Client.vendorId = '" . $vendorId . "' AND Survey_Log.surveyId=" . $conn->quote($survey["objectId"]) . " ORDER BY Client.name LIMIT 200";
  }
  ...
  ///
  /// Email template selection based on email type
  ///
  ...
  // EXAMPLE: To enable for vendor ID 42:
  // $inlineScoreVendorIds = [42];

  // EXAMPLE: To enable for multiple vendors:
  // $inlineScoreVendorIds = [42, 99, 156];

  // INSTRUCTIONS:
  // 1. User provides vendor ID
  // 2. Claude adds to array above
  // 3. Deploy to production
  // 4. Send reminders (only specified vendors get inline scores)
  // 5. Remove from array when done
  // 6. Deploy again
  // ============================================

  // ==============================================================================
  // TEMPLATE SELECTION STRATEGY: "Single-Link First, Inline Optional"
  // ==============================================================================
  // INITIAL EMAIL: Always single "Share Your Feedback" button
  //                - Simple, lightweight format for maximum inbox placement
  //                - Direct link to webview score selection page (&webview=1)
  //                - No bot protection needed (first touchpoint)
  //
  // REMINDER EMAIL: Single button by default, inline scores optional
  //                DEFAULT: Single button (matches initial email)
  //                  - Uses email_template_reminder_single.html
  //                  - Direct link to webview (&webview=1)
  //                  - Same flow as initial email
  //                OPTIONAL: Inline NPS buttons (vendor-specific)
  //                  - Uses email_template_reminder.html
  //                  - Shows 0-10 score buttons directly in email
  //                  - Uses URL hash fragments (#score=X) for bot protection
  //                  - Two-step confirmation (click score → Submit Score button)
  // ==============================================================================
  ...
  // Email html and base subject (will be personalized per recipient in loop)
  if($emailType == $GLOBALS["EMAIL_TYPE"]["SURVEY"]) {   //Survey
    ...
    $emailTemplate = str_ireplace("{{EMAIL_HTML}}",$html,$emailTemplate);
    $baseSubject = $survey["emailSubjectReminder"];
  }
  ...
  while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    if($row["vendorId"]===$vendorId && isset($row["name"]) && isset($row["email"]) && filter_var($row["email"], FILTER_VALIDATE_EMAIL)) {
      ...
      $sesReq['Destination']['ToAddresses'] = array(str_ireplace(',',' ',$row["name"]) . " <{$row['email']}>");
      ...
      $emailBody = str_ireplace("{{ClientFullName}}",$row["name"],$emailBody);
      ...
      // Personalize subject line with ClientContact
...
function sendEmployeeSurvey(){
  ...
  if($emailType==$GLOBALS["EMAIL_TYPE"]["SURVEY"]) {         //Survey
    ...
    $sql = "SELECT sl.objectId,sl.vendorId, CONCAT(u.firstName, ' ',u.lastName) as name, u.email FROM Survey_Log sl " .
           ...
           "sl.score IS NULL AND u.active=1 AND u.isDeleted=0 AND sl.vendorId='$vendorId' AND sl.surveyId='$surveyId' ORDER BY u.firstName LIMIT 1";
  }

  else {                      //Reminder
    $sql = "SELECT sl.objectId,sl.vendorId, CONCAT(u.firstName, ' ',u.lastName) as name, u.email FROM Survey_Log sl " .
           "INNER JOIN User u ON sl.staffId = u.objectId " .
           "WHERE sl.emailStatus=2 AND sl.reminderDate IS NOT NULL AND sl.reminderDate <= UTC_TIMESTAMP() AND sl.reminderDate >= UTC_TIMESTAMP() - INTERVAL 30 DAY AND " .
...
function sendPulseSurvey(){
  ...
  if($emailType==$GLOBALS["EMAIL_TYPE"]["SURVEY"]) {         //Survey
    $sql = "SELECT Client.objectId as clientId, Client.vendorId, Client.title, Client.name, Client.organisation, Client.email, Client.drl," .
           ...
           "INNER JOIN Survey_Log ON Client.objectId = Survey_Log.clientId WHERE Client.active=1 AND Client.sendSurveyEmail=1 AND Survey_Log.score IS NULL AND " .
           "Survey_Log.emailStatus=0 AND Survey_Log.sendDate IS NOT NULL AND DATE(Survey_Log.sendDate) = CURDATE() AND " .
           "Client.vendorId = '" . $vendorId . "' AND Survey_Log.surveyId=" . $conn->quote($survey["objectId"]) . " ORDER BY Client.name LIMIT 1";
  }

  else {                      //Reminder
    $sql = "SELECT Client.objectId as clientId, Client.vendorId, Client.title, Client.name, Client.organisation, Client.email, Client.drl," .
           "CONCAT(User.firstName, ' ',User.lastName) as drlName, Survey_Log.objectId as objectId  from Client INNER JOIN User ON Client.drl = User.objectId " .
           "INNER JOIN Survey_Log ON Client.objectId = Survey_Log.clientId WHERE Client.active=1 AND Client.sendSurveyEmail=1 AND Survey_Log.additionalQuestions IS NULL AND " .
           "Survey_Log.emailStatus=2 AND Survey_Log.reminderSentOnDate IS NULL AND Survey_Log.reminderDate IS NOT NULL AND Survey_Log.reminderDate <= UTC_TIMESTAMP() AND Survey_Log.reminderDate >= UTC_TIMESTAMP() - INTERVAL 30 DAY AND " .
           "Client.vendorId = '" . $vendorId . "' AND Survey_Log.surveyId=" . $conn->quote($survey["objectId"]) . " ORDER BY Client.name LIMIT 200";
  }
...
function sendTriageSurvey() {
  ...
  if($emailType==$GLOBALS["EMAIL_TYPE"]["SURVEY"]) {         //Survey
    ...
    $sql = "SELECT Client.objectId as clientId, Client.vendorId, Client.title, Client.name, Client.organisation, Client.email, Client.drl," .
           "CONCAT(User.firstName, ' ',User.lastName) as drlName, Survey_Log.objectId as objectId  from Client INNER JOIN User ON Client.drl = User.objectId " .
           "INNER JOIN Survey_Log ON Client.objectId = Survey_Log.clientId WHERE Client.active=1 AND Client.sendSurveyEmail=1 AND Survey_Log.triageScore IS NULL AND " .
           "Survey_Log.emailStatus=2 AND Survey_Log.reminderSentOnDate IS NULL AND Survey_Log.reminderDate IS NOT NULL AND Survey_Log.reminderDate <= UTC_TIMESTAMP() AND Survey_Log.reminderDate >= UTC_TIMESTAMP() - INTERVAL 30 DAY AND " .
           "Client.vendorId = '" . $vendorId . "' AND Survey_Log.surveyId=" . $conn->quote($survey["objectId"]) . " ORDER BY Client.name LIMIT 50";
  }
  ...
  while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    if($row["vendorId"]===$vendorId && isset($row["name"]) && isset($row["email"]) && filter_var($row["email"], FILTER_VALIDATE_EMAIL)) {
      ...
      //$sesReq['Destination']['ToAddresses'] = array("Tabraiz Dada <tjdada@gmail.com>");
      //$sesReq['Destination']['ToAddresses'] = array("Tabraiz Dada <tabraiz@outlook.com>");
      //$sesReq['Destination']['ToAddresses'] = array("Greg Tilse <gtilse@clientculture.com.au>");
